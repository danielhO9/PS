...
void pollardRho(__int128_t n, vector<__int128_t>& ret) {
	// cout << (ll) n << '\n';
	if (n == 1) return;
	if (n % 2 == 0) {
		ret.push_back(2);
...
vector<__int128_t> factorization(__int128_t n) {
	vector<__int128_t> ret;
	pollardRho(n, ret);
	return ret;
}
...
void solve() {
	ll n; cin >> n;
	n = (__int128_t) n;
	vector<__int128_t> ans = factorization(n);
	set<__int128_t> S;
