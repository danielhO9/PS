...
int main() {
    ...
    for (int i = 0; i < M; ++i) {
        int a; cin >> a;
        v.push_back(Structure {N, i, i, --a});
    }
    for (int i = 0; i < K; ++i) {
        ll r, s, e, c; cin >> r >> s >> e >> c;
        ll h = N - r + 1;
        v.push_back(Structure {h, --s, --e, --c});
    }
    sort(v.begin(), v.end(), [](const Structure& a, const Structure& b) {
        return a.h < b.h;
    });
    LazyPropagation seg {(int) M};
    for (auto s: v) {
        ll rem = seg.query(s.s, s.e);
        ans[s.c] += s.h * (s.e - s.s + 1) - rem;
        seg.update(s.s, s.e, s.h);
    }
