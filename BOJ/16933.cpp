...
void solve() {
	...
	for (int i = 0; i < 1000; ++i) for (int j = 0; j < 1000; ++j) for (int k = 0; k < 11; ++k) dist[i][j][k] = 987654321;
	...
	while (!Q.empty()) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (board[ny][nx]) {
				if (broked < K) {
					if (d % 2 == 0 && dist[ny][nx][broked + 1] > d + 2) {
						Q.push({{ny, nx}, broked + 1});
						dist[ny][nx][broked + 1] = d + 2;
					} else if (d % 2 == 1 && dist[ny][nx][broked + 1] > d + 1) {
						Q.push({{ny, nx}, broked + 1});
						dist[ny][nx][broked + 1] = d + 1;
					}
				}
			} else {
				if (dist[ny][nx][broked] > d + 1) {
					Q.push({{ny, nx}, broked});
	...
	vector<int> tmp;
	for (int i = 0; i <= K; ++i) if (dist[N - 1][M - 1][i] != 987654321) tmp.push_back(dist[N - 1][M - 1][i]);
	if (tmp.empty()) cout << -1;
