...
void query(int num, int t) {
	...
	if (nr < 0 || nr >= N || nc < 0 || nc >= N || chess[nr][nc] == 2) {
		...
		int tr = r[num], tc = c[num], tidx = idx[num];
		vector<int> V;
		while (tokens[tr][tc].size() > tidx) {
			int tmp = tokens[tr][tc].back(); tokens[tr][tc].pop_back();
			V.push_back(tmp);
		}
		while (!V.empty()) {
			int tmp = V.back(); V.pop_back();
			idx[tmp] = tokens[nr][nc].size();
			tokens[nr][nc].push_back(tmp); r[tmp] = nr; c[tmp] = nc;
		}
	} else {
		int tr = r[num], tc = c[num], tidx = idx[num];
		while (tokens[tr][tc].size() > tidx) {
